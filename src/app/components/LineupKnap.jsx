...
const FollowButton = ({ onClick }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      gsap.to(container, { duration: 0.3, scale: 1.05 });
      gsap.to(content, { duration: 0.3, scale: 1.05 });
    };
  ...
  return (
    <Link href="/bands" passHref>
      <button className="flex justify-center items-center text-center px-4 text-nowrap relative w-56 h-20 sm:w-56 sm:h-20 cursor-pointer">
        <div ref={containerRef} onClick={onClick} className="absolute cursor-pointer inset-0 flex justify-center items-center">
          <div ref={contentRef} className="absolute bg-knap-10 cursor-pointer flex justify-center items-center h-16 w-52 sm:w-52 text-sm rounded-xl border-2 hover:border-blue-600 border-gray-500"></div>
